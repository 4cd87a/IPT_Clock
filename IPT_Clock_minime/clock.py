...
class App(QWidget):
    def __init__(self, states):
        ...
        self.label.setFont(QFont('Arial',int(self.frameGeometry().height()/labelFontCoeff)))
        self.label.setContentsMargins(0,2*int(self.frameGeometry().height()/countDownFontCoeff),0,0)
        ...
        self.clock_time = QStackedLayout()
        self.clock_time.setStackingMode(1)


        self.clock_time.addWidget(self.countDown)
        self.clock_time.addWidget(self.m)
        self.clock_time.addWidget(self.label)

        # self.rightLayout.addWidget(self.label)
        self.rightLayout.addLayout(self.clock_time)
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_P or e.key() == Qt.Key_Space:
...
class AnalogClock(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if audio_play and 9.5<(self.duration-self.elapsedTime)<10 and not self.stimerOn and self.sound:
            self.stimerOn = True
        ...
        if not(abs(currentAngle) > 2 * math.pi):
            ...
            self.parent.countDown.setText(tr)
    ...
    def switchPause(self,paused=None):
        ...
        if self.paused:
            ...
            self.paused = True
            if audio_play and self.stimerPlay != None: self.stimerPlay.stop()
            self.startPause = datetime.datetime.now()
